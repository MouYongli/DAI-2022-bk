...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="PyTorch Single")
    ...
    parser.add_argument('--num-epochs', type = int, default=50, help = 'number epochs of training on client between communication')

    parser.add_argument('--dp', type = str, default='none', help='differential privacy type: none, gaussian, laplacian')
    args = parser.parse_args()

    exp_dir = osp.join(here, "../../Experiments", f"d_{args.num_data_mnist}_{args.num_data_svhn}_{args.num_data_usps}_{args.num_data_synth}_{args.num_data_mnistm}", "single", f"e_{args.num_epochs}")
    ...
    for ep in range(args.num_epochs):
        ...
        for client_idx in range(num_clients):
            ...
            print("============ {} ============".format(datasets[client_idx]))
            if args.log:
                logfile.write("============ {} ============\n".format(datasets[client_idx]))
            ## training local model
            model, train_loader, optimizer = client_model_set[client_idx], train_loaders[client_idx], optimizers[
                client_idx]
            model.train()
            num_data = 0
            correct = 0
            ...
            with open(osp.join(exp_dir, 'log.csv'), 'a') as f:
                log = [ep, datasets[client_idx], '', 'train', train_loss, train_acc]
                log = map(str, log)
                f.write(','.join(log) + '\n')
            if args.log:
                logfile.write(
            ...
            for test_idx, test_loader in enumerate(test_loaders):
                ...
                with open(osp.join(exp_dir, 'log.csv'), 'a') as f:
                    log = [ep, datasets[client_idx], datasets[test_idx], 'test', test_loss, test_acc]
                    log = map(str, log)
                    f.write(','.join(log) + '\n')
                if args.log:
