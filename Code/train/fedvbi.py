    ...
    for rd in range(args.num_rounds):
        ...
        with torch.no_grad():
            ...
            for m in module_set:
                ...
                for client_idx in range(num_clients):
                    client_global_model_set[client_idx].state_dict()[m+"_mu"].data.copy_(new_mu[m+"_mu"])
                    client_global_model_set[client_idx].state_dict()[m+"_log_sigma"].data.copy_(torch.log_(new_sigma[m+"_log_sigma"]))
        ...
        for test_idx, test_loader in enumerate(test_loaders):
            ...
            for batch_idx, (data, target) in enumerate(test_loader):
                data, target = data.to(device), target.to(device)
                num_data += target.size(0)
                output = client_global_model_set[client_idx].eval()(data)
